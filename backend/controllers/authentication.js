...
const AuthenticationController = {
    ...
    verifyRefreshToken: async (req, res) => {
        ...
        if (decoded) {
            ...
            return res.status(401).json({ valid: false, message: 'Invalid or expired refresh token' });
        }
    ...
    loginWithEmail: async (req, res) => {
        ...
        if (user && await AuthUtils.comparePassword(password, user.password)) {
            const tokens = AuthUtils.issueTokens(user);
            Utils.setRefreshTokenCookie(res, tokens.refreshToken);
    ...
    signup: async (req, res) => {
        ...
        const {plainToken, hashedToken} = AuthUtils.createEmailVerificationToken();
    ...
    try {
        ...
        const result = await pool.query(
            ...
            VALUES ($1, $2, $3, $4, $5, $6) 
            RETURNING id`,
            [username, email, hashedPassword, display_name, hashedToken, tokenExpiration]
        );

        const newUserId = result.rows[0].id;

        // 4. Send the email with the PLAIN token and the USER ID
        await sendVerificationEmail(email, plainToken, newUserId);
    ...
    try {
        ...
        if (user.email_verification_token_expires_at < currentTime) {
        ...
        const isValid = await AuthUtils.comparePassword(token, user.email_verification_token);
    ...
    const { plain_token, hashed_token } = AuthUtils.createEmailVerificationToken(); // Re-using the same generator is fine
    ...
    try {
        ...
        const result = await pool.query(
            'SELECT id, password_reset_token, password_reset_expires_at FROM users WHERE id = $1', 
            [id]
        );
        ...
        // 3. Verify Token Hash
        const isValid = await AuthUtils.comparePassword(token, user.password_reset_token);
        if (!isValid) {
